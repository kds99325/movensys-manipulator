...
try:
    ...
    print("cv2 / numpy module not found")
try:
...
# numpy dtype / channel count per sensor_msgs/Image encoding
_ENCODING_MAP = {
...
def imgmsg_to_array(msg) -> "np.ndarray":
    """
    Convert a sensor_msgs/Image message to a numpy array (cv_bridge-free,
    equivalent to imgmsg_to_cv2 with desired_encoding='passthrough').

    Reshapes the raw byte buffer according to the message encoding, honoring
    the row stride (``msg.step``) so padded rows decode correctly.
    """
    encoding = msg.encoding.lower()
    dtype, channels = _ENCODING_MAP.get(encoding, (np.uint8, 1))
    dtype = np.dtype(dtype)
    if getattr(msg, "is_bigendian", 0):
        dtype = dtype.newbyteorder(">")

    itemsize = dtype.itemsize
    row_elems = msg.step // itemsize if msg.step else msg.width * channels
    buf = np.frombuffer(msg.data, dtype=dtype).reshape(msg.height, row_elems)
    buf = buf[:, : msg.width * channels]  # drop row padding
    return buf if channels == 1 else buf.reshape(msg.height, msg.width, channels)
...
def save_image_from_rosbag(
    ...
    Notes:
        This function reads messages in a specified topic, decodes them into
        numpy/OpenCV images (via numpy for raw Image, cv2.imdecode for
        CompressedImage), and saves them as PNG/JPG files.
    """
    ...
    for bag in args.rosbag :
        ...
        with AnyReader([bag_path]) as reader:
            ...
            if (
                ...
                save_image_from_rosbag(reader, conn, msg_type)
