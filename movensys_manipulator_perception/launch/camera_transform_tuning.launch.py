...
def generate_launch_description():
    pkg_share = get_package_share_directory('movensys_manipulator_isaac_ros_config')

    xacro_file = os.path.join(get_package_share_directory('movensys_manipulator_description'),
        'urdf', os.environ.get('MANIPULATOR_MODEL', 'dobot_cr3a'), 'movensys_manipulator.xacro')
    ...
    declare_child_frame = DeclareLaunchArgument(
        ...
        default_value=PythonExpression([
            "'camera_nvblox_color_optical_frame' if '", use_sim_time, "'.lower() == 'true' else 'camera_nvblox_link'"
        ]),
    ...
    camera_transform_tuning_node = Node(
        package='movensys_manipulator_isaac_ros_config',
