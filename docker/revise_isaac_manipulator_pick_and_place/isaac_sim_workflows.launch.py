...
def get_robot_state_publisher(workflow_config: IsaacSimWorkflowConfig) -> Node:
    ...
    robot_state_publisher = Node( # 이 노드의 역할은 TF를 publish한다.
...
def get_camera_info_publisher_node(use_sim_time: bool) -> Node:
    '''Camera infor publisher node that publishes camera_info for Isaac Sim cameras
    when Isaac sim doesn't provide them.
    ...
    return Node(
            ...
            parameters=[{
                ...
                'cx': 960.0,
                'cy': 600.0,
                'rgb_frame_id': 'front_stereo_camera_left',
                'depth_frame_id': 'front_stereo_camera_left',
            }],
        )
...
def launch_setup(context, *args, **kwargs):
    ...
    if workflow_config.use_sim_time:
        ...
        manipulator_init_nodes.append(
                get_camera_info_publisher_node(workflow_config.use_sim_time))
